# configs/experiments/lstm_hybrid.py
"""
...
# Train
python train.py --config configs/experiments/lstm_hybrid.py --skip-sanity

# Finding the best thresholds on the VAL dataset
python optimize_thresholds.py \
    --config configs/experiments/lstm_hybrid.py \
    --trials 50 \
    --objective sharpe

# Evaluate | Default ["test", "backtest"]
python evaluate.py --config configs/experiments/lstm_hybrid.py --mode test

# Backtest
python evaluate.py --config configs/experiments/lstm_hybrid.py --mode backtest

# Evaluate "test" + final model
python evaluate.py \
  --config configs/experiments/lstm_hybrid.py \
  --checkpoint output/lstm_hybrid/models/final.pth \
  --mode test
...
python optimize.py --config configs/experiments/lstm_hybrid.py --trials 50

# Compare
python compare.py

# rm -r output/lstm_hybrid; rm -rf data_cache/*
"""
from configs.base_config import MasterConfig, LSTMConfig

cfg = MasterConfig(experiment_name="lstm_hybrid")

# ── Backbone ──────────────────────────────────────────────────────────────────
# backbone=lstm  params=146.8K
cfg.backbone.type = "lstm"
cfg.backbone.lstm = LSTMConfig(
    hidden_size   = 64,
    num_layers    = 2,
    dropout       = 0.2,   # inter-layer dropout; only applies when num_layers > 1
    bidirectional = True,
)
# ── Loss ──────────────────────────────────────────────────────────────────────
